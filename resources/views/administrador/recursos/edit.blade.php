...
<body>
  ...
  <form action="{{ route('administrador.recursos.update', $recurso->id_recurso) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="group">
      <label for="titulo">Título *</label>
      <input type="text" id="titulo" name="titulo" value="{{ old('titulo', $recurso->titulo) }}" required>
      @error('titulo')<div class="error">{{ $message }}</div>@enderror
    </div>
    <div class="group">
      <label for="descripcion">Descripción</label>
      <textarea id="descripcion" name="descripcion">{{ old('descripcion', $recurso->descripcion) }}</textarea>
    ...
    <div class="group">
      ...
      <select name="fk_tipo_recurso" id="fk_tipo_recurso" required>
        @foreach($tipos as $t)
          <option value="{{ $t->id_tipo_recurso }}" @selected(old('fk_tipo_recurso', $recurso->fk_tipo_recurso) == $t->id_tipo_recurso)>{{ $t->nombre }}</option>
        @endforeach
      </select>
    ...
    <div class="group">
      ...
      <input type="url" id="enlace" name="enlace" placeholder="https://..." value="{{ old('enlace', $recurso->enlace) }}">
    ...
    <div class="group">
      ...
      @if($recurso->enlace)
        <p style="margin-top:6px; font-size:13px;">Actual: <a href="{{ $recurso->enlace }}" target="_blank" rel="noopener">Abrir enlace</a></p>
      @endif
    </div>
    <div class="actions">
