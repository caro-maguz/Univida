...
<body>
  <div class="container">
    ...
    <form action="{{ route('test.procesar') }}" method="POST" id="testForm">
      @csrf
      ...
      @foreach($preguntas as $index => $pregunta)
        @if($pregunta->categoria !== $categoriaActual)
          @php $categoriaActual = $pregunta->categoria; @endphp
          <div style="background: {{ $categoriaColors[$categoriaActual] ?? '#e0e0e0' }}; padding: 0.8rem 1rem; border-radius: 10px; margin: 1.5rem 0 1rem 0; font-weight: bold; color: #333; font-size: 1.05rem;">
            {{ $categoriaIcons[$categoriaActual] ?? '📋' }} 
            @if($categoriaActual === 'General')
              Bienestar General
            @else
              Violencia {{ $categoriaActual }}
            @endif
          </div>
        @endif
          ...
          <div class="opciones">
            <div class="opcion" style="flex: 1;">
              <input type="radio" 
                     ...
                     value="si" 
                     id="p{{ $pregunta->id_pregunta }}_si"
                     onchange="actualizarProgreso()">
              <label for="p{{ $pregunta->id_pregunta }}_si" style="font-size: 1.1rem;">
                <strong>SÍ</strong>
              </label>
            </div>
            <div class="opcion" style="flex: 1;">
              <input type="radio" 
                     name="respuestas[{{ $pregunta->id_pregunta }}]" 
                     value="no" 
                     id="p{{ $pregunta->id_pregunta }}_no"
                     onchange="actualizarProgreso()">
              <label for="p{{ $pregunta->id_pregunta }}_no" style="font-size: 1.1rem;">
                <strong>NO</strong>
              </label>
            </div>
          </div>
