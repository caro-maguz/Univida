{{-- resources/views/centro_recursos.blade.php --}}
<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
...
<body>
  ...
  <main>
    <article class="card">
      ...
      <ul>
        @forelse($protocolos as $item)
          <li class="list-group-item">
            <div>
              <h6>{{ $item->titulo }}</h6>
              <p>{{ $item->descripcion }}</p>
            </div>
            <a href="{{ route('recursos.download', $item->id_recurso) }}" class="btn-descargar">Descargar</a>
          </li>
        @empty
      ...
      {{-- === SECCIÓN 3: MATERIAL EDUCATIVO === --}}
      <h4 class="text-success">📗 Material Educativo</h4>
      <ul>
        @forelse($educativos as $item)
          <li class="list-group-item">
            <div>
              <h6>{{ $item->titulo }}</h6>
              <p>{{ $item->descripcion }}</p>
