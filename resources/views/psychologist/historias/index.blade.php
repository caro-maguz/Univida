...
<body>
    <div class="top-bar">
        ...
        <a href="{{ route('dashboard.psychologist') }}" class="back">← Volver al panel</a>
    </div>
        ...
        <tbody>
            @forelse($historias as $historia)
                <tr>
                    ...
                    <td>
                        <div class="actions">
                            <a class="btn-small btn-view" href="{{ route('psychologist.historias.show', $historia->id) }}"><i class="fas fa-eye"></i> Ver</a>

                            @if($historia->estado === 'pendiente')
                                <form action="{{ route('psychologist.historias.aprobar', $historia->id) }}" method="POST" style="display:inline">
                                    @csrf
                                    <button type="submit" class="btn-small btn-approve"><i class="fas fa-check"></i> Aprobar</button>
                                </form>

                                <form id="rechazar-{{ $historia->id }}" action="{{ route('psychologist.historias.rechazar', $historia->id) }}" method="POST" style="display:inline">
                                    @csrf
                            ...
                            <a class="btn-small btn-edit" href="{{ route('psychologist.historias.edit', $historia->id) }}"><i class="fas fa-edit"></i> Editar</a>
                            <form action="{{ route('psychologist.historias.destroy', $historia->id) }}" method="POST" style="display:inline" onsubmit="return confirm('¿Eliminar esta historia?')">
                                @csrf
                                @method('DELETE')
