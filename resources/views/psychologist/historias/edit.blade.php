...
<body>
    <a href="{{ route('psychologist.historias.index') }}" class="back"><i class="fas fa-arrow-left"></i> Volver</a>
    <h1>Editar Historia #{{ $historia->id }}</h1>

    <form action="{{ route('psychologist.historias.update', $historia->id) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="group">
            <label for="contenido">Contenido de la historia *</label>
            <textarea id="contenido" name="contenido" required>{{ old('contenido', $historia->contenido) }}</textarea>
            @error('contenido')<div class="hint" style="color:#c62828">{{ $message }}</div>@enderror
        ...
        <div class="actions">
            <a href="{{ route('psychologist.historias.index') }}" class="button cancel"><i class="fas fa-times"></i> Cancelar</a>
    ...
    <div class="row">
        <div class="box">
            ...
            <p style="margin:6px 0"><strong>Fecha de envío:</strong> {{ $historia->created_at->format('d/m/Y H:i') }}</p>
            @if($historia->usuario)
                <p style="margin:6px 0"><strong>Usuario:</strong> {{ $historia->usuario->nombre ?? 'Anónimo' }}</p>
            @endif
        </div>
    </div>
</body>
